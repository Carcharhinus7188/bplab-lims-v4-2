...
st.set_page_config(page_title="BPLab LIMS",page_icon="🧪",layout="wide",initial_sidebar_state="expanded")
st.markdown("""
<style>
:root{color-scheme:light!important;--bp-navy:#12364a;--bp-blue:#176b87;--bp-cyan:#3aa6b9;--bp-line:#dce7ee;--bp-muted:#6d8290}
...
.bp-card{background:white;border:1px solid var(--bp-line);border-radius:17px;padding:18px 19px;box-shadow:0 7px 20px rgba(18,54,74,.07);height:100%}
...
def header(subtitle):
    st.markdown(f'<div class="bp-header"><div class="bp-company">{COMPANY_CN}</div><div class="bp-en">{COMPANY_EN}</div><div class="bp-system">样品全过程追溯与受控实验记录系统</div><div>{APP_VERSION} · {subtitle}</div></div>',unsafe_allow_html=True)
...
# Persistent login through an expiring token stored in URL query parameters.
...
nav=ROLE_MENUS[role]
with st.sidebar:
    st.markdown("## BPLab LIMS")
    st.caption("Sample Lifecycle & Controlled Records")
...
if page=="首页看板":
    ...
    c3.metric("检测中",sum(x["status"] in ["已分配","检测中"] for x in samples))
    c4.metric("待复核",sum(x["status"]=="待复核" for x in samples))
    c5.metric("待回库",sum(x["status"]=="待回库确认" for x in samples))
    section("样品实时状态")
    if samples:
        df=pd.DataFrame(samples)
        st.dataframe(df[["sample_no","sample_name","status","location","owner","qty_current","unit","due_date","updated_at"]],hide_index=True,use_container_width=True)
    else:st.info("暂无样品，请由收样员执行样品入库")
    section("流程总览")
    st.markdown('<div class="bp-card">样品入库 → 待分配 → 任务分配 → 实验员领取 → 检测中 → 原始记录提交 → 待复核 → 复核完成 → 样品归还 → 回库确认 → 留样保存</div>',unsafe_allow_html=True)

elif page=="样品入库":
    header("样品接收、登记和首次入库")
    sample_no=st.text_input("样品编号",value=next_sample_no())
    a,b,c=st.columns(3)
    client=a.text_input("客户名称")
    sample_name=b.text_input("样品名称")
    model=c.text_input("规格型号")
    batch_no=a.text_input("批号")
    qty=b.number_input("接收数量",min_value=0.0,value=1.0)
    unit=c.text_input("单位",value="件")
    received=a.date_input("接收日期",value=date.today())
    due=b.date_input("计划完成日期",value=date.today())
    location=c.text_input("首次入库位置",value="样品库A-待分配区")
    exps=st.multiselect("检测项目",list(EXPERIMENTS))
    notes=st.text_area("样品状态/备注")
    if st.button("确认接收、入库并创建检测任务",type="primary",use_container_width=True):
        if not sample_name or not exps:st.error("样品名称和检测项目不能为空")
        else:
            create_sample({"sample_no":sample_no,"client":client,"sample_name":sample_name,
              "model":model,"batch_no":batch_no,"qty_received":qty,"unit":unit,
              "received_date":str(received),"location":location,"due_date":str(due),"notes":notes},exps,username)
            st.success("样品已入库，检测任务已生成")
            st.rerun()
    ...
    if not samples:st.info("暂无样品")
        ...
        a.metric("当前状态",s["status"]);b.metric("当前位置",s["location"]);c.metric("当前负责人",s["owner"] or "未指定");d.metric("当前数量",f"{s['qty_current']} {s['unit']}")
        section("样品基本信息")
        st.dataframe(pd.DataFrame([s])[["sample_no","client","sample_name","model","batch_no","qty_received","qty_current","unit","received_date","due_date","notes"]],hide_index=True,use_container_width=True)
        section("流程时间轴")
        events=sample_events(selected)
        for i,e in enumerate(events):
            css="bp-current" if i==len(events)-1 else "bp-done"
            st.markdown(f'<div class="{css}"><b>{e["created_at"]} · {e["action"]}</b><br>{e["from_status"] or "起点"} → {e["to_status"]}<br>位置：{e["from_location"] or "—"} → {e["to_location"] or "—"}<br>操作人：{e["actor"]}　{e["details"] or ""}</div>',unsafe_allow_html=True)
        section("关联检测任务")
        ts=[x for x in list_tasks() if x["sample_no"]==selected]
        if ts:st.dataframe(pd.DataFrame(ts)[["task_no","experiment","assignee","reviewer","status","room","updated_at"]],hide_index=True,use_container_width=True)

elif page=="任务分配":
    header("检测任务分配")
    tasks=list_tasks(statuses=["待分配","已分配"])
    if not tasks:st.info("暂无待分配任务")
    else:
        st.dataframe(pd.DataFrame(tasks)[["task_no","sample_no","sample_name","experiment","assignee","reviewer","status"]],hide_index=True,use_container_width=True)
        ...
        room=c.text_input("检测位置/房间",value="检测室")
        if st.button("保存任务分配",type="primary",use_container_width=True):
            assign_task(task_no,assignee,reviewer,room,username)
            st.success("任务已分配，样品状态看板已立即更新")
            st.rerun()

elif page=="我的检测任务":
    header("领取样品、进入检测和完成后归还")
    tasks=list_tasks(assignee=username)
    if not tasks:st.info("暂无分配给你的任务")
    else:
        st.dataframe(pd.DataFrame(tasks)[["task_no","sample_no","sample_name","experiment","status","room","sample_status"]],hide_index=True,use_container_width=True)
        selected=st.selectbox("选择任务",[x["task_no"] for x in tasks])
        t=task(selected)
        if t["status"]=="已分配":
            if st.button("领取样品并开始检测",type="primary",use_container_width=True):
                set_task_status(selected,"检测中")
                update_sample(t["sample_no"],username,"实验员领取样品",status="检测中",location=t["room"],owner=username,details=f"用于{t['experiment']}")
                st.rerun()
        ...
        if s["status"]=="待归还":
            ...
            proposed=st.text_input("建议回库位置",value="样品库A-留样区")
            ...
            if st.button("提交样品归还",type="primary",use_container_width=True):
                ...
                st.success("已提交回库确认，所有人状态看板立即可见")
                st.rerun()
    ...
    if not rows:st.info("暂无待回库样品")
        ...
        location=st.text_input("确认回库位置",value=rr["proposed_location"])

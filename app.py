...
if page=="首页看板":
    ...
    if not task_list:
        ...
        with tabs[5]:
            ...
            payload={
                ...
                "equipment_snapshot":business.get("equipment_checks") or [],
    ...
    if rs:
        ...
        if r["status"] in ["待检测员确认","退回检测员"] and username==r["tester"]:
            category=st.text_input("检验类别",r.get("report_category") or "委托检验");statement=st.text_area("样品情况说明",r.get("sample_statement") or "");conclusion=st.text_area("检验结论",r.get("conclusion") or "");notes=st.text_area("需说明情况",r.get("notes") or "")
